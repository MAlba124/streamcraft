//! pf-av1 — the AV1 (AOMedia Video 1) codec plugin.
//!
...
//! (pf-vp8 carried the identical stale blurb while being production-complete). Judged
...
//!   roundtrip test the way pf-vp8 has, no external fixtures required.
...
//! This is the AV1 analogue of the way pf-vp8 handles `show_frame == 0`.
//!
//! The upstream decoder is broad, but our raw-video vocabulary
//! ([`profluens_video`]) names only four 8-bit pixel formats. [`Av1Dec`] therefore
...
//!
//! | AV1 output                     | profluens pixfmt | status                    |
//! |--------------------------------|--------------------|---------------------------|
...
//!
//! When [`profluens_video`] grows `i422` / `i444` / `i420_10` etc., the map here is
//! the only thing that changes — the element contract does not.
//!
//! Nativization debt, tracked in PLAN.md:
//! - **Mandatory `oxideav-core` dep**: unlike pf-vp8 (zero-dep), `oxideav-av1` pulls
...
//!   registry bridge would make this crate zero-transitive-dep like pf-vp8. Flagged
//!   for an upstream contribution.
//! - **Decode into pool memory**: the upstream decoder returns owned `Vec` planes;
//!   [`Av1Dec`] pays one plane copy into pool memory per frame (the same cost pf-vp8
