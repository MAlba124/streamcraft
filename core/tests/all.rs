//! The single integration-test binary for profluens-core (spec: Testing — "tests are
...
// is empty, so `cargo test -p profluens-core` (feature off) still links and passes.
#[path = "introspect.rs"]
mod introspect;
