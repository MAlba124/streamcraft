//! pf-mp4 — a hand-written, dependency-free ISO base media file format (ISO-BMFF, ISO/IEC
//! 14496-12) MP4 container **demuxer** for profluens.
//!
...
//!
//! Layout mirrors the `pf-mkv` plugin — the nearest analog (a hand-written container with a
...
//!   ([`codec::family_for`]). It **reuses `pf-mkv`'s** `nal_head_from_config` + [`Reframer`]
...
//! - [`Mp4Demux`] — the profluens demuxer **element** ([`element`]): a progressive MP4 byte
...
pub use pf_mkv::{nal_head_from_config, Reframer, ReframeError};
...
/// None`, so [`Registry::parse`](profluens_core::registry::Registry::parse) cannot build
